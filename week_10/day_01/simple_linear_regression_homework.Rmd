...
* Plot the data, taking `estimated_length` as the independent (x) variable and `actual_length` as the dependent (y) variable. 
...
```
The correlation coefficient is 0.8, which is a strong correlation.
...
the regression coefficient of estimated_length is 1.22, so for every unit increase in actual length, the estimated length increases by 1.22. The R^2 value of 0.64 suggests that 64% of the data in this graph can be explained by a linear regression confidently.
...
The values for the estimated_length are significant, however the intercept is not.. not sure what this means.
