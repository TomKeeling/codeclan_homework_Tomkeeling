...
housing <- read_csv("data/kc_house_data.csv") %>% 
  ...
  select(-c(date, id, zipcode, sqft_living15, sqft_lot15, yr_renovated))
...
```{r}
lm(price ~ ., data = housing) %>% 
  alias()
```
we can remove sqfoot_basement then
...
```{r}
mod1a <- lm(price ~ sqft_living, data = housing)
mod1b <- lm(price ~ bathrooms, data = housing)
...
```
```{r}
houses_tidy_numeric_res <- houses_tidy_numeric %>%
  add_residuals(model_1) %>% 
  select(-sqft_living, -price)

ggpairs(houses_tidy_numeric_res)
```
```{r}
mod2a <- lm(price ~ sqft_living + yr_built, data = housing)
mod2b <- lm(price ~ sqft_living + lat, data = housing)
