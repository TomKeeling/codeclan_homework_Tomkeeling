...
```{r}
str_detect(whole_text, "[Tt]hought [Pp]olice")
...
```{r}
big_brother <- str_extract_all(whole_text, "(i?)Big Brother") 
length(big_brother[[1]])
...
```{r}
date_pattern <- "[A-Z][a-z]+ [0-9]*th, [0-9]+"
whole_text %>% 
...
```{r}
whole_text <- str_replace_all(whole_text, "Winston", "David")
whole_text
...
#this is'nt robust- need to explore pattern after just to check!
```
...
whole_text %>% 
  ...
  str_replace_all("department", "section") %>% 
  str_replace_all("Departmenst", "Sections") %>% 
  str_replace_all("departments", "sections") %>% 
  str_replace_all("ministry", "department") %>% 
  str_replace_all("Ministry", "Department") %>% 
  str_replace_all("ministries", "departments") %>% 
  str_replace_all("Ministries", "Departments") %>% 
  str_replace_all("Mini", "Dep") %>%
  str_replace_all("mini", "dep")
  
  #not that keen ngl - could've been a little more creative. plurals for example, can be done as Department -> Ministrie or Ministrie -> Department. Whole thing can shrink a little xo

whole_text %>% 
  str_extract_all("Department|Ministry|Ministrie|Mini")
```
